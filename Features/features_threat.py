...
import requests
import re
from Features.features_common import COMMON_PATHS
...
def check_sensitive_file_disclosure(url, verbose=False):
    """Check for exposure of sensitive files like .htaccess, web.config, .DS_Store."""
    from urllib.parse import urljoin
    files = ["/.htaccess", "/web.config", "/.DS_Store", "/.bash_history"]
    found = []
    for path in files:
        ...
        try:
            ...
            if r.status_code == 200 and len(r.content) > 10:
                print(f"\n[!] Sensitive file exposed: {test_url}")
                found.append(test_url)
        except Exception:
            continue
    return found

def check_test_pages(url, verbose=False):
    """Check for common test/dev pages."""
    from urllib.parse import urljoin
    test_pages = ["/test", "/test.php", "/dev", "/dev.php", "/staging", "/staging.php"]
    found = []
    for path in test_pages:
        ...
        try:
            ...
            if r.status_code == 200:
                print(f"\n[!] Test/dev page exposed: {test_url}")
                found.append(test_url)
        except Exception:
            continue
    return found

def check_exposed_readme_license(url, verbose=False):
    """Check for exposed README or LICENSE files."""
    from urllib.parse import urljoin
    files = ["/README", "/README.md", "/LICENSE", "/LICENSE.txt"]
    found = []
    for path in files:
        test_url = urljoin(url, path)
        try:
...
def check_common_paths(base_url):
    """Return a list of accessible sensitive paths."""
    findings = []
    ...
    for path in COMMON_PATHS:
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                findings.append(f"Sensitive path accessible: {test_url}")
        except requests.exceptions.RequestException:
            continue
    return findings

def check_directory_listing(url):
    """Returns True if directory listing is enabled, else False."""
    try:
        test_url = url.rstrip('/') + '/'
...
def check_open_redirect(url, verbose=False):
    """Returns True if open redirect is found, else False."""
...
def main():
    ...
    sensitive_files = check_sensitive_file_disclosure(url, verbose=args.verbose)
    if sensitive_files:
        for f in sensitive_files:
            threats.append(f"Sensitive file exposed: {f}")
    test_pages = check_test_pages(url, verbose=args.verbose)
    if test_pages:
        for page in test_pages:
            threats.append(f"Test/dev page exposed: {page}")
    exposed_files = check_exposed_readme_license(url, verbose=args.verbose)
    if exposed_files:
        for file in exposed_files:
            threats.append(f"Exposed file: {file}")
